...
export default function MarkdownRenderer({ content, isUser = false }: MarkdownRendererProps) {
  ...
  const codeBg = isUser ? 'bg-white/20' : 'bg-gray-100';
  const codeTextColor = isUser ? 'text-white' : 'text-gray-800';
  const borderColor = isUser ? 'border-white/30' : 'border-gray-300';
  const blockquoteBorder = isUser ? 'border-white/50' : 'border-gray-300';
  const blockquoteBg = isUser ? 'bg-white/10' : 'bg-gray-50';
